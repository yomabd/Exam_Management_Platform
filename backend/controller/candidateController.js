...
exports.getCandidateExams = async (req, res) => {
  try {
    ...
    const exams = await QuestionBank.find({ _id: { $in: user.assignedExams } });
    ...
    console.error("Error fetching candidate exams:", error.message);
    res.status(500).json({ message: "Server error" });
  }
};

exports.getCandidateExamById = async (req, res) => {
  try {
    const userId = req.user?._id;
    const { id } = req.params;
    ...
    const exam = await QuestionBank.findById(id);
    if (!exam) {
      return res.status(404).json({ message: "Question bank not found!" });
    }

    if (!exam.assignedCandidates.includes(userId)) {
      return res
        .status(403)
    ...
    console.error("Error fetching an exam by ID:", error.message);
